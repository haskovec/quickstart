...
call ant -f %WORKSPACE%/blacktie/test/initializeBlackTie.xml -DJBOSS_HOME=%JBOSS_HOME% -DBT_HOME=%WORKSPACE%\blacktie\target\dist\ -DVERSION=%NARAYANA_CURRENT_VERSION% -DMACHINE_ADDR=%MACHINE_ADDR% -DJBOSSAS_IP_ADDR=%JBOSSAS_IP_ADDR% -DBLACKTIE_DIST_HOME=%BLACKTIE_DIST_HOME%  -Dbasedir=%WORKSPACE% initializeDatabase initializeJBoss initializeBlackTie -debug
...
call %WORKSPACE%\blacktie\target\dist\blacktie-%NARAYANA_CURRENT_VERSION%\setenv.bat
